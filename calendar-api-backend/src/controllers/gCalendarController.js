...
gCalendarRouter.get('/login', async (req, res) => {
    ...
    console.log(`GCalendar login 1.3: Redirecting user to Google OAuth2 login page: ${url}`);
...
gCalendarRouter.get('/redirect', async (req, res) => {
    ...
    oauth2Client.getToken(code, async (err, tokens) => {
        ...
        console.log(`GCalendar login 2.6: Received tokens for user ${JSON.stringify(tokens)}`);
        oauth2Client.setCredentials(tokens);
        userTokens = tokens;
        try {
...
gCalendarRouter.post('/days-shifts-to-gcal', verifyUserAuth, async (req, res) => {
    ...
    console.log(`gCalendarController day's shifts to gcal 1: Adding all shifts to GCal for date ${date}. Request from user ${req.user.email}`);
    try {
        const result = await gCalendarService.addDaysShiftsToGcal(date);
        console.log(`gCalendarController day's shifts to gcal 2: Shifts added to GCal for date ${date}`);
