          ...
          {sortedCalendar.map((user, idx) => (
            <div key={idx} className="flex">
              <div
                ...
                className={`p-2 flex items-center border-b ${userHasGcal(user, gCalendarEvents) ? 'h-20' : 'h-12'}`}
                style={{ width: '12%' }}
              >
                <Avatar>
              ...
              <div id="shifts-data-column" className="flex-1 overflow-x-auto">
                <div id="visual-grid-wrapper" className="relative">
                  <div
                    className={`grid absolute inset-0 grid border-b ${userHasGcal(user, gCalendarEvents) ? 'h-20' : 'h-12'}`}
                ...
                <div id="shifts-wrapper" className="grid" style={{ gridTemplateColumns: 'repeat(48, minmax(0, 1fr))' }}>
                  {user.shifts.map((shift, idx) => {
                    const start = calculateGridColumnStart(shift.dtstart, shift.dateRequested);
                    const span = calculateGridColumnSpan(shift.dtstart, shift.dtend, shift.dateRequested);
                ...
                {userHasGcal(user, gCalendarEvents) ? (<div id="gcalendar-wrapper" className="grid h-8" style={{ gridTemplateColumns: 'repeat(48, minmax(0, 1fr))' }}>
                  {gCalendarEvents
                    .find((calUser: CalendarUser) => Number(calUser.slingId) === user.id)
                        ...
                        <HoverCard key={idx}>
                          ...
                          <HoverCardContent className="w-full max-w-md p-6 grid gap-6">
                            {/* <div className="flex justify-between space-x-4">
                              <div className="space-y-1">{startEndPretty(event.start.dateTime, event.end.dateTime)}</div>
                              <div className="text-xs font-semibold">Event Details</div>
                            ...
                            <div className="flex items-start gap-4">
                              ...
                              <div className="grid gap-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  {event.recurringEventId && (<RepeatIcon className="w-5 h-5 text-muted-foreground" />)}
                                </div>
