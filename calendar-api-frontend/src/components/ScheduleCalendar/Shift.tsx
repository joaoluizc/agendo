...
export function Shift(props: ShiftProps) {
      ...
      const onUp = () => {
        ...
        setPendingChange({
          ...
          summary: latest.deleteIntent
            ? `${position.name} · ${clock.hourRange(base)}`
            : `${position.name} · ${clock.hourRange(base)} → ${clock.hourRange(latest)}`,
        });
      };
   ...
   *
   * The time shows from 2 hours rather than 3, so someone reading their own day finds it on
   * the block instead of only on hover. A 24-hour range always fits there; a long 12-hour
   * one ("11:30 AM – 1:30 PM") can end in an ellipsis on a screen narrower than ~1600px.
   */
  const widths = (span.end - span.start) * 2;
  const mode =
    widths >= 4 ? "full" : widths >= 3 ? "label" : widths >= 2 ? "tight" : "code";
